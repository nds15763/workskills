...
排查 = 在一堆猜想里用最少验证次数找真因。决断不是"选一个答案"，是给每个猜想刷状态机，并写清证据覆盖到哪一层。入口咒语：**把任何结论翻成预测——"如果它成立，数据里该看到什么、不该看到什么？"**

先定**问题层级**（一层套一层，证据收窄再进下一层）：
...
```text
Hx：<写成因果命题，不写状态；粗猜想必须拆到机制>
  先验：__（来源：历史事故 / commit 改动面 / 运行日志 / 架构约束；来源不清 -> 低权重）
  若为真应看到：__
  若为假也可能看到：__
  诊断性证据：__（能把它和别的猜想分开那条；两猜想下都出现 = 似然比≈1 = 不能更新）
  当前证据：__
  状态：排除 / 走弱 / 挂起 / 领先 / 确认
  边界：这条证据只覆盖到哪一层
```

五态状态机（焊到上面四栏裁决：确认←对的、领先/挂起/走弱←缺证据、排除←应丢弃；比四栏多两道闸）：
...
下一步 = 选 `诊断性 × 结果不确定性 ÷ 成本` 最大的**最低成本验证**，不是验最顺手的。

三道防假排除闸（缺一就把"排除"降级为"走弱"）：

- 代理证据 ≠ 目标证据：fixture 过只排 fixture 面、validate 过只证格式、文件存在 ≠ 用户看到有效贴纸。
- 整包排除禁令：证据打到哪层只能排到哪层；"网络/VLM/UI 排除了"先拆机制再说。
- 领先 ≠ 确认：单样本 / 跨次方差大的领先不能登记成根因（Lucy 路径就停在这）。

反向证伪一趟（结论取反、倒推最小杀链集）交给 `truth-condition-checker`，本 skill 只画状态和证据覆盖层。
