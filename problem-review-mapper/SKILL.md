...
- 先翻译用户现象，再选择证据投影。实现侧词汇只说明“去哪看证据”，不能替代用户概念或目标 claim。
...
- 主 agent 做编排和决策；运行验证、独立取证时派 subagent。
- 例子只能作为 case lens：允许抽取“主干/证据/反例/退出旧说法”等可迁移实践，禁止把例子里的业务形状、字段、工具或架构写成通用准则。
...
flowchart LR
  ...
  B --> B1["证据投影<br/>权威源 / 运行 / 实现"]
  C --> C1["依据<br/>PRD / spec / 设计"]
  D --> D1["风险<br/>冲突 / 旧知识 / 漏洞"]
  E --> E1["状态<br/>已证实 / 推断 / 待验证"]
  F --> F1["下一步<br/>subagent / writeback / 验证"]
```

文字回答只保留三类内容：

- 一句话结论：图表达不了的最终 verdict。
- 关键证据：权威来源、用户裁决、运行窗口、实现定位或文档来源。
- 落盘说明：写了什么、索引到哪里、还缺什么验证。

## 从材料到图

当输入是一大段文字、PRD、运行记录、设计稿、实现 diff 或会议材料时，先做最小抽取：
...
hypotheses:
  - id: h1
    ...
    prior_source: "<历史经验 / 改动面 / 运行记录 / 架构约束>"
...
| 分支 | 放什么 |
|---|---|
| 证据分支 | 权威来源、用户裁决、运行记录、实现位置、文档来源 |
| 条件分支 | 如果、是否、成功/失败、开关、环境条件 |
| 细节分支 | 子步骤、动作对象、参与角色、关键事件 |
| 裁决分支 | 对、不对、缺证据、应丢弃 |
| 产物分支 | 修正、文档落点、索引、验证、回滚 |
...
```mermaid
flowchart LR
  A["用户动作<br/>已证实<br/>权威来源"] -->|产生| B["阶段产物<br/>当前证据投影"]
  B -->|供给| C["目标链<br/>共通 claim"]
  C -->|冲突| D["旧解释<br/>应丢弃"]
  C -->|缺证据| E["运行/用户观察<br/>待验证"]
```
...
```mermaid
flowchart LR
  M["主 agent<br/>定义问题、拆证据面、回收裁决"] --> A["实现证据 agent"]
  M --> B["运行 / 验证 agent"]
  M --> C["文档 / 知识 agent"]
  M --> D["配置 / 环境 agent"]
...
- 查什么证据面。
- 不查什么，避免重复。
- 输出必须包含权威来源、运行窗口、实现定位、文档来源或用户裁决。
...
| 类别 | 含义 | 输出要求 |
|---|---|---|
| 对的 | 已被目标证据、权威来源或运行证据支持 | 写入结论，可作为下一步依据 |
...
- 领先 ≠ 确认：单样本 / 跨次方差大的领先，别当根因直接改实现（Lucy 路径就停在这）。
