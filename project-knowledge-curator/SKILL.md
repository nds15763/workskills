...
## 知识治理轴与 canonical kind

Curator 不重定义 canonical `kind`。`kind` 回答 concept / claim 的结构类型；Curator 只回答这条知识能不能用、证据强度如何、归入哪类知识索引。

| 字段 | 归属 | 回答的问题 |
|---|---|---|
| `kind` | `canonical-claim-compiler` | 这个对象/命题的结构类型是什么：concept / state / action / rule / relation / event / metric |
| `claim_type` | Curator | 这条知识在认识论上是什么：fact / rule / inference / value / taste / vision / decision |
| `knowledge_kind` | Curator / Wiki | 这条知识归入哪个业务知识面：prd / constraint / architecture |
| `knowledge_color` | Curator | 默认能不能进上下文：white / gray / black |
| `authority_level` | Curator | 证据来源强度：A / B / C / D |

三色描述“默认使用策略”，`claim_type` 描述“认识论性质”，`knowledge_kind` 描述“索引归属”。三者正交，必须同时存在；它们都不能替代 canonical `kind`。
