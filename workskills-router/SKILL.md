...
- **执行边界**：执行前先把每个 Loop 节点分类为确定性的代码或可执行检查、agent 判断与推理、独立验证或人工 gate；不要让角色边界隐含在提示词里。
- **结果归属**：确定性检查的结果不得再交给 agent 重新判断；agent 只处理仍需语境、权衡或解释的部分。
- **失败路由**：失败必须输出结构化原因与下一步路由或终止条件；不得盲目重试或重跑。
- **证据闭合**：可执行检查通过，只关闭其声明的证据层；除非该检查声明的对象就是目标真值，否则不能据此关闭目标 gate。
