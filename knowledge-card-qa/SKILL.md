...
```mermaid
flowchart LR
  A["权威源<br/>用户裁决 / 文档 / 运行证据 / 实现证据"] --> B["知识卡<br/>人话入口"]
  B --> C["快速理解"]
...
- 如果输入来自用户现象或表象反馈，先写“用户看见/期待什么”和“共通概念/claim 是什么”，再写证据。实现侧名词只能作为证据投影，不能当卡片主语。
...
- 用户要理解项目、OpenSpec change、任务计划、实现链路或业务机制。
...
| 涉及运行/验证证据 | 派 subagent 取证，主 agent 只回收并裁决 |
...
| 证据 | 用户确认 / 权威文档 / OpenSpec / 运行证据 / 实现证据；Obsidian 知识优先给 `[[功能点]] + claim_ref`；事实依据和价值偏好分区 |
| 失效条件 | 哪些事实变化后必须重验 |
...
现状：<用户目标、文档、实现或路书现在真实是什么状态。>
改动：<本轮新增、删除、修正了什么；包括明确抛弃的旧说法。>
证据：<权威源 / 用户裁决 / 运行验证 / 实现位置 / Knowledge Pack；Obsidian 知识写 [[功能点]] + claim_id/source_ref。>
...
证据：
<用户确认、权威文档、OpenSpec、运行证据、实现证据。Obsidian 知识优先写业务域、#业务标签、[[功能点]]、claim_id / claim_ref 和 [[页面#^block-id]]；只有路径不够。>

失效条件：
<哪些权威源、契约、任务状态、实现位置或运行证据变化后，这张卡必须重验。>
```
...
- 先讲用户场景和共通概念，再讲实现名词；必要时才落到表、RPC、字段等证据投影。
...
- 不要只列实现名词、字段名、表名、Todo；用户无法据此判断场景。
...
- 默认实时捞取，不落卡。
- 高频、高风险、已裁决的困惑才存成卡。
- 卡片只做理解入口，不能替代用户裁决、docs、OpenSpec、实现证据或运行证据。
- 每张存储卡必须有证据和失效条件。
...
如果卡片无法反查权威源、用户裁决、运行证据或实现位置，就只能作为临时解释，不能落成稳定卡。
