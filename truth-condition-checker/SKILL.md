---
name: truth-condition-checker
description: >-
  Use when 用户要校验 claim、gate、decision、验收口径、事故结论、设计判断或方案取舍是否成立，
  或需要把一句判断拆成可验证条件、找矛盾、找缺证据、找反例时使用。
---

# Truth Condition Checker

本 skill 把判断拆成真值条件：什么为真时它成立，什么为假时它失败，哪些条件互相矛盾。

它不负责发明方案，也不负责把不合法语法强行验证。若对象、关系、状态还没成句，先用 `logical-grammar`。

## 输入前置检查

router 已经做了一道分流，但 router 可能判错或被绕过。本 skill 在开工前必须自检一次——这是冗余兜底，不是 router 的替代：

- 输入是 `高级 / 自然 / 有趣 / 应该 / 更好` 等价值判断时，**先转 `say-show-boundary`**——价值不能写真值条件，只能写取向、约束、可观察后果。
- 输入的对象/关系/状态还没成句时，**先转 `logical-grammar`** 改写，再回来拆条件。
- 输入还没确定 `concept_id` / `claim_id`，或问题是“这是不是同一条 claim”时，**先转 `canonical-claim-compiler`**。本 skill 校验 truth condition，不裁决 identity。
- 输入是开放式问题陈述（"我们要不要做 X"）时，**先转 `problem-statement-card`** 收窄。

本 skill 只处理已经成句、已经剥层的可证伪命题。

## 核心分工

| 输入 | 处理 |
|---|---|
| claim | 拆成事实条件、证据条件、反例条件 |
| gate | 拆成放行条件、拒绝条件、未知条件 |
| decision | 拆成选择成立条件、退出条件、风险条件 |
| incident conclusion | 拆成观测事实、因果条件、排除项 |
| acceptance criteria | 拆成可观测信号和失败信号 |
| accepted canonical claim | 校验 `truth_condition`、`local_truth_hash`、`closure_truth_hash` 与证据是否一致 |

## 判断流程

```mermaid
flowchart LR
  A["claim / gate / decision"] --> B["拆条件<br/>C1 C2 C3"]
  B --> C["标类型<br/>事实 / 规则 / 推断 / 价值"]
  C --> D["找证据<br/>代码 / 日志 / 数据 / 文档"]
  D --> E["找反例<br/>每个条件什么会让它为假"]
  E --> R["反向证伪 pass<br/>结论取反，倒推最小杀链集"]
  R --> F["找矛盾<br/>条件互斥 / 证据冲突"]
  F --> G["裁决<br/>成立 / 不成立 / 缺证据 / 语义不合法"]
```

`找反例` 是逐叶子的局部证伪（单看 C1 什么时候假）。`反向证伪 pass` 是整条链的全局证伪（假设结论错，倒推哪个最小条件集必须为假）。一条链可以正向全过却反向一推就塌——只有反向走一趟才抓得到。

## 条件拆分

每个条件必须能回答三件事：

| 字段 | 要求 |
|---|---|
| 条件 | 用可验证句子写，不写感受词 |
| 证据 | 指向代码、日志、配置、数据、文档或用户明确裁决 |
| 失败方式 | 写出什么发生时该条件为假 |

如果写不出失败方式，这个条件通常不是事实条件，可能是价值、愿景或偏好，交给 `say-show-boundary`。

## 反向证伪 pass（独立一趟，必走）

正向拆条件是「凑齐条件 → 成立」的确认语气。这一趟是反过来：**假设结论是错的，倒推要让它错，哪些条件必须为假。** 弄不假的才真正可以依赖。

做法：

1. **结论取反**：把原判断改写成它的否定（C → ¬C），从这里往回走。
2. **每个节点标支撑类型**——决定它怎么被杀：
   - `AND-支撑`：结论靠 `P1 ∧ P2 ∧ …`（所有前提同时成立）。**任一前提为假，结论即假**。这是最常见、最脆的形态。
   - `OR-支撑`：结论靠 `P1 ∨ P2 ∨ …`（冗余/双保险）。**必须所有前提都为假，结论才假**；杀一个不算数。
3. **用与非门统一推假**：每个 AND 节点等价于一个 NAND——`结论的否定 = NAND(它的前提)`，只要前提不全真，否定就成立。顺着否定往叶子推，列出**最小杀链集**：能独立把结论翻假的最小条件组合。
   - AND-支撑节点 → 每个单前提都是一个 1 元杀链集。
   - OR-支撑节点 → 杀链集是「该节点全部前提同时为假」这一整组。
4. **给每个杀手条件标证据强度**：杀手条件当前是 `已证为真 / 弱证据 / 未知`。**一个 AND-支撑链里出现「未知或弱证据」的杀手条件 = 这条链正向看着稳、实则一碰就碎**，这是本 pass 要抓的主猎物。

| 危险信号 | 含义 |
|---|---|
| 单个 AND 杀手条件证据=未知 | 整条结论挂在一个没验证的点上，正向审查看不出来 |
| 所有杀链集都需要「多条件同时为假」 | 链有真实冗余，比较稳；别误当脆弱去砍 |
| 把 OR-支撑当 AND 砍 | 误杀冗余设计——禁止动作，见下 |

## 输出格式

```md
结论：成立 / 不成立 / 缺证据 / 条件冲突 / 不是事实命题

原判断：
<claim / gate / decision>

Claim ref（如有）：
<claim_id + local_truth_hash + closure_truth_hash；没有则写 pending / none>

真值条件：
| 条件 | 类型 | 当前证据 | 反例/失败方式 | 状态 |
|---|---|---|---|---|
| C1 | fact/rule/inference/value | ... | ... | 真/假/未知 |

反向证伪（结论取反，倒推最小杀链集）：
| 杀链集（让结论为假的最小条件组合） | 节点支撑 | 杀手条件证据强度 | 危险？ |
|---|---|---|---|
| {C1 为假} | AND | 未知/弱/已证 | 是/否 |
| {C3∧C4 同时为假} | OR | ... | ... |

矛盾：
- <条件 A> 与 <条件 B> 冲突，因为 ...

裁决：
- 可以继续依赖：
- 不能继续依赖：
- 需要补证：
```

## 矛盾类型

| 类型 | 例子 |
|---|---|
| 条件互斥 | 同一对象同时要求 `created` 和 `not_created` |
| 层级冲突 | 用 UI 展示状态证明后端持久状态 |
| 时间冲突 | 用变更后的日志证明变更前的事故 |
| 证据冲突 | 代码路径和运行日志指向不同结论 |
| 量词过强 | 从“一次成功”推出“总是成功” |
| 伪事实 | 把“应该更高级”当成可证伪 claim |

## Gate 检查

对 gate 不要只问“能不能过”，要拆成：

- 放行条件：全部为真才允许继续。
- 阻断条件：任一为真就必须停。
- 未知条件：缺证据时不能冒充已通过。
- 退出条件：后续哪个事实变化会让 gate 失效。

## Decision 检查

对 decision 先写清楚“选择成立条件”，再写清楚“不选什么”：

- 该选择依赖哪些事实为真？
- 哪些事实为假时必须换方案？
- 它替代或丢弃了哪个旧判断？
- 是否存在价值判断伪装成事实条件？

## 禁止动作

- 不要用“我感觉合理”当证据。
- 不要把未知条件默认为真。
- 不要只找支持证据；必须写反例或失败方式。
- 不要只做正向确认；必须走一趟反向证伪 pass（结论取反、倒推最小杀链集）。
- 不要把 NAND 落成「任一前提为假 = 链死」的钝刀；先标每个节点是 AND-支撑还是 OR-支撑，OR-支撑要全部前提为假才算杀，否则会误杀冗余设计。
- 不要用兜底实现掩盖条件冲突；先解释为什么冲突会出现。
- 不要把价值、审美、愿景硬拆成事实真值；交给 `say-show-boundary`。
- 不要给 pending / disputed claim 直接下“成立”结论；先让 Curator 裁决语义状态。
