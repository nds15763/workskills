...
| 检查 | 路书里的落点 |
|---|---|
| `logical-grammar` | spine / runtime / gate 节点必须有合法对象、动作、状态和所有权；对象域不清不能升首环 |
| `truth-condition-checker` | 每个 gate、转绿依据、锚点裁决都必须有 pass/fail 条件和反例 |
| `say-show-boundary` | 愿景、审美、方案倾向只能做材料/说明或设计约束，不能直接驱动颜色 |
...
- `say/show` 边界：价值、审美、愿景、方案偏好可以放材料/说明节点或锚点约束；它们不能作为转绿证据，必须转成可观察后果或由用户裁决为取向。
...
- pass：全部为真才允许后续节点推进或转绿。
- fail：任一为真必须阻断。
- unknown：缺证据时保持黄/红，不能冒充 pass。
- invalidation：后续哪个事实变化会让 gate 失效并回退。
