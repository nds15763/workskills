...
## 范例 7B：外部 Goal / Ralph Loop 必须落成子环

场景：Codex 进度面板或 Claude Code Ralph Loop 本轮显示 5 个动作已完成：

```
回收 6 个证据面
创建母 OpenSpec 文档包
更新 roadmap canvas/spec
跑 validate / audit
收口快问快答自检
```

这些动作不能直接画成父 runtime 链的 5 个 sibling 后逐个转绿。正确做法是：把它们压成当前 runtime 节点下的一个内部子环，并让该子环作为本轮事务整体变绿。

```
当前 runtime 节点: review包出口 [黄或绿，按外层证据传播]
  ↓
Round1 执行回退子环 [绿]
  回收证据面 [绿]
    → 重建母 OpenSpec [绿]
...
1. 在当前 runtime 节点下创建 `Round1 / Goal step / Ralph iteration N` 子环。
2. 子环节点、edge、group 几何、父节点颜色传播作为同一批 `.canvas` 事务写入。
3. 子环全绿只表示“这一轮执行闭合”；父 runtime / spine 是否转绿继续由外层 gate 和证据决定。
4. Codex Goal 只有在 `.canvas` 写回、`audit.py` 通过、快问快答自检完成后才能 `complete`。
5. Claude Code `/ralph-loop` 只有在子环写回、父节点传播、`audit.py` 通过后才能输出 completion promise；否则写 `partial / blocked / awaiting human`。

反例：

```
Codex progress 面板显示 5 个 checkmark，于是把父 workblock 改绿。
```

违规：progress checkmark 只是执行投影，不是 roadmap 证据。必须先落内部子环；父环是否变绿由外层闭环传播决定。
